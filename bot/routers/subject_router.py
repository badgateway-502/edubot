...
@subject_router.message(SubjectState.lecture_opened, F.text == "Текущая лекция")
@subject_router.message(SubjectState.lecture_current_opened, F.text == "Следующая лекция")
@subject_router.message(Lab.lab_opened, F.text == "Обратно к лекции")
@subject_router.message(SubjectState.lecture_current_opened, F.text == "Следующая лекция")
@subject_router.message(Quiz.variants_question | Quiz.scalare_question | Quiz.moderfile_question | Quiz.finished_quiz,
                        F.text == "Обратно к лекции")
async def select_current_lecture(message: types.Message, state: FSMContext):
    ...
    if message.text == "Следующая лекция":
        future_api.next_lecture()

    cur_lecture = future_api.get_current_lecture_db(message.from_user.id, (await state.get_data())['subject_id'])
    keyboard = keyboard_for_current_lecture(any(cur_lecture['questions']))
    # TODO ученик сдал лабу, прошел тест
    if cur_lecture['video_file_id']:
        await message.answer_video(cur_lecture['video_file_id'])
    if cur_lecture['description_file_id']:
        await message.answer_document(cur_lecture['description_file_id'])
    if cur_lecture['text_description']:
        await message.answer(cur_lecture['text_description'])
...
async def some_lecture(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if cur_lecture['video_file_id']:
            await message.answer_video(cur_lecture['video_file_id'], reply_markup=keyboard_for_old_lecture())
        if cur_lecture['description_file_id']:
            await message.answer_document(cur_lecture['description_file_id'], reply_markup=keyboard_for_old_lecture())
        if cur_lecture['text_description']:
            await message.answer(cur_lecture['text_description'], reply_markup=keyboard_for_old_lecture())
